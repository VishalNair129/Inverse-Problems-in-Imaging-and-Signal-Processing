...
%
%
% This code is a modified version of the spatial TV reconstruction method
% detailed by Tom Goldstein and Stanley Osher. The Split Bregman Method for
...
uk        = zeros(rows,cols,height);
uk(1,1,1) = 6; uk(1,2,1 )= -1; uk(2,1,1) = -1;
uk(rows,1,1) = -1; uk(1,cols,1) = -1;
uk(1,1,2) = -1; uk(1,1,height) = -1;
uk        = fftn(uk);
uk        = mu + lambda*uk;
...
for outer = 1:nout
    ...
    for inner=1:nin
        ...
        % Solve the linear system (formulation 4.2 of the split bregman paper) in the Fourier domain
        u       = ifftn(fftn(rhs)./uk);
