...
x=double(x);       % we convert the image to double precision
x=imresize(x,0.4); %To avoid dealing with computer expense difficulties, we downsize the image 
                   % to a lower size (50% its original size).
...
eta = 0.1;%05;%01       % Relative noise level.
delta = eta*norm(b_ex);
...
%L-curve
figure;
rlcurve= l_curve(U,s,b,'tsvd');

%let's truncate at the r=3500
rPicard=2340;
...
%let's truncate at the r from L-curve
x_rlcurve = tsvd(U,s,V,b,rlcurve);
figure;
imagesc(reshape(x_rlcurve,N,N)), colormap gray;
axis image off
%c = clim;
title('TSVD r from L-curve')
...
%[x_lambdalcurve,rho,eta] = tikhonov(U,s,V,b,lambda);
x_lambdalcurve = tikhonov(U,s,V,b,lambda);
figure;
imagesc(reshape(x_lambdalcurve,N,N)), colormap gray;
axis image off
%c = clim;
title('Tikhonov \lambda from L-curve')
...
imagesc(reshape(x_lambdalcurve,N,N)), colormap gray;
axis image off
title('Tikh. \lambda from L-curve')
